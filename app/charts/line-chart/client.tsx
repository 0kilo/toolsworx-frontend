...
import { renderLineChart, exportLineChartSVG, exportLineChartJSON, type LineChartData } from "@/lib/tools/logic/charts/chart-line"
import toolContent from "./line-chart.json"

export default function LinechartClient() {
  const [data, setData] = useState<LineChartData>(toolContent.exampleData as LineChartData)
  const chartRef = useRef<HTMLDivElement>(null)

  const renderChart = useCallback(() => {
    if (!chartRef.current || !data.data.length) return
    renderLineChart(chartRef.current, data)
  }, [data])
  ...
  const handleDownload = () => {
    ...
    exportLineChartSVG(chartRef.current, data.title)
  }

  const handleExportJSON = () => {
    exportLineChartJSON(data)
  }
