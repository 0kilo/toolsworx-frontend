...
import { renderScatterChart, exportScatterChartSVG, exportScatterChartJSON, type ScatterChartData } from "@/lib/tools/logic/charts/chart-scatter"
import toolContent from "./scatter-chart.json"

export default function ScatterchartClient() {
  const [data, setData] = useState<ScatterChartData>(toolContent.exampleData as ScatterChartData)
  const chartRef = useRef<HTMLDivElement>(null)

  const renderChart = useCallback(() => {
    if (!chartRef.current || !data.data.length) return
    renderScatterChart(chartRef.current, data)
  }, [data])
  ...
  const handleDownload = () => {
    ...
    exportScatterChartSVG(chartRef.current, data.title)
  }

  const handleExportJSON = () => {
    exportScatterChartJSON(data)
  }
