...
import { renderAreaChart, exportAreaChartSVG, exportAreaChartJSON, type AreaChartData } from "@/lib/tools/logic/charts/chart-area"
import toolContent from "./area-chart.json"

export default function AreachartClient() {
  const [data, setData] = useState<AreaChartData>(toolContent.exampleData as AreaChartData)
  const chartRef = useRef<HTMLDivElement>(null)

  const renderChart = useCallback(() => {
    if (!chartRef.current || !data.data.length) return
    renderAreaChart(chartRef.current, data)
  }, [data])
  ...
  const handleDownload = () => {
    ...
    exportAreaChartSVG(chartRef.current, data.title)
  }

  const handleExportJSON = () => {
    exportAreaChartJSON(data)
  }
