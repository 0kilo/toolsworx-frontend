...
import { calculateLorentzTransformation } from "@/lib/tools/logic/unit-conversions/spacetime"
import toolContent from "./spacetime.json"
...
export default function SpaceTimeConverterPage() {
  ...
  const handleCalculate = () => {
    ...
    try {
      const result = calculateLorentzTransformation({ x: Number(x), t: Number(t), v: Number(v) })
      setXPrime(result.xPrime.toExponential(6))
      setTPrime(result.tPrime.toExponential(6))
      setGamma(result.gamma.toFixed(6))
      setBeta(result.beta.toFixed(6))
    } catch (error) {
