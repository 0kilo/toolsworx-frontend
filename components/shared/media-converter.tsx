...
interface MediaConverterProps {
  title: string
  description: string
  fromFormats: MediaFormat[]
  toFormats: MediaFormat[]
  defaultFrom?: string
  defaultTo?: string
  maxSize?: number
}
...
export function MediaConverter({
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const job = await amplifyApiClient.convertMedia(file, toFormat)
      ...
      const pollInterval = setInterval(async () => {
        try {
          const status = await amplifyApiClient.getMediaJobStatus(job.id)
  ...
  const handleDownload = async () => {
    ...
    try {
      const blob = await amplifyApiClient.downloadMediaJob(conversion.jobId)
